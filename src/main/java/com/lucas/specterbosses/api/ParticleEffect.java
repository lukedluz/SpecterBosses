...
public enum ParticleEffect
{
    ...
    public void display(final float offsetX, final float offsetY, final float offsetZ, final float speed, final int amount, final Location center) throws ParticleVersionException, ParticleDataException, IllegalArgumentException {
        final List<Player> players = (List<Player>)Lists.newArrayList();
        final List<Player> list;
        center.getWorld().getNearbyEntities(center, 20.0, 20.0, 20.0).forEach(d -> {
        ...
        this.display(offsetX, offsetY, offsetZ, speed, amount, center, players);
    }
    ...
    public void display(final Vector direction, final float speed, final Location center) throws ParticleVersionException, ParticleDataException, IllegalArgumentException {
        final List<Player> players = (List<Player>)Lists.newArrayList();
        final List<Player> list;
        center.getWorld().getNearbyEntities(center, 20.0, 20.0, 20.0).forEach(d -> {
            if (d instanceof Player) {
    ...
    public static final class ParticlePacket
    {
        ...
        public static void initialize() throws VersionIncompatibleException {
            ...
            try {
                ParticlePacket.version = Integer.parseInt(Character.toString(Reflection.PackageType.getServerVersion().charAt(3)));
                if (ParticlePacket.version > 7) {
                    ParticlePacket.enumParticle = Reflection.PackageType.MINECRAFT_SERVER.getClass("EnumParticle");
                }
                final Class<?> packetClass = Reflection.PackageType.MINECRAFT_SERVER.getClass((ParticlePacket.version < 7) ? "Packet63WorldParticles" : "PacketPlayOutWorldParticles");
                ParticlePacket.packetConstructor = Reflection.getConstructor(packetClass, (Class<?>[])new Class[0]);
                ParticlePacket.getHandle = Reflection.getMethod("CraftPlayer", Reflection.PackageType.CRAFTBUKKIT_ENTITY, "getHandle", (Class<?>[])new Class[0]);
                ParticlePacket.playerConnection = Reflection.getField("EntityPlayer", Reflection.PackageType.MINECRAFT_SERVER, false, "playerConnection");
                ParticlePacket.sendPacket = Reflection.getMethod(ParticlePacket.playerConnection.getType(), "sendPacket", Reflection.PackageType.MINECRAFT_SERVER.getClass("Packet"));
            }
        ...
        private void initializePacket(final Location center) throws PacketInstantiationException {
            ...
            try {
                ...
                if (ParticlePacket.version < 8) {
                    ...
                    Reflection.setValue(this.packet, true, "a", ParticlePacket.enumParticle.getEnumConstants()[this.effect.getId()]);
                    ...
                    if (this.data != null) {
                        ...
                        Reflection.setValue(packet, declared, fieldName, value);
                    }
                }
